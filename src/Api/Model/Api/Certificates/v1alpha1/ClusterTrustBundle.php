 ...
 * cluster can read ClusterTrustBundles by impersonating a serviceaccount that they have access to.
 *
 * It can be optionally associated with a particular assigner, in which case it contains one valid set
 * of trust anchors for that signer. Signers may have multiple associated ClusterTrustBundles; each is
 * an independent set of trust anchors for that signer. Admission control is used to enforce that only
 * users with permissions on the signer can create or modify the corresponding bundle.
 */
...
class ClusterTrustBundle
{
    ...
    protected string $apiVersion = 'certificates.k8s.io/v1alpha1';

    #[Kubernetes\Attribute('kind')]
    protected string $kind = 'ClusterTrustBundle';

    #[Kubernetes\Attribute('metadata', type: AttributeType::Model, model: ObjectMeta::class)]
    protected ObjectMeta $metadata;

    #[Kubernetes\Attribute('spec', type: AttributeType::Model, model: ClusterTrustBundleSpec::class, required: true)]
    protected ClusterTrustBundleSpec $spec;

    public function __construct(string|null $name, string $trustBundle)
    {
        $this->metadata = new ObjectMeta($name);
        $this->spec = new ClusterTrustBundleSpec($trustBundle);
    }
    ...
    {
        return $this->spec->getTrustBundle();
    }
    ...
    {
        $this->spec->setTrustBundle($trustBundle);
     ...
     */
    public function getSpec(): ClusterTrustBundleSpec
    {
        return $this->spec;
    }
     ...
     * @return static
     */
    public function setSpec(ClusterTrustBundleSpec $spec): static
    {
        $this->spec = $spec;
