 ...
 * addresses. An IP address can be represented in different formats, to guarantee the uniqueness of the
 * IP, the name of the object is the IP address in canonical format, four decimal digits separated by
 * dots suppressing leading zeros for IPv4 and the representation defined by RFC 5952 for IPv6. Valid:
...
#[Kubernetes\Operation('delete', path: '/apis/networking.k8s.io/v1beta1/ipaddresses/{name}')]
...
#[Kubernetes\Operation('list-all', path: '/apis/networking.k8s.io/v1beta1/ipaddresses', response: IPAddressList::class)]
class IPAddress
{
    ...
    #[Kubernetes\Attribute('spec', type: AttributeType::Model, model: IPAddressSpec::class)]
    protected IPAddressSpec $spec;

    public function __construct(string|null $name, ParentReference $parentRef)
    {
        $this->metadata = new ObjectMeta($name);
        $this->spec = new IPAddressSpec($parentRef);
    }
    ...
    {
        $this->spec->setParentRef($parentRef);
     ...
     */
    public function getSpec(): IPAddressSpec
    {
        return $this->spec;
    }
     ...
     * @return static
     */
    public function setSpec(IPAddressSpec $spec): static
    {
        $this->spec = $spec;
